...
| **MQTT v5 / vendor TCP** | (device-only) | binary | IoT device telemetry — *not* public (`Modules/DeviceGateway.md`) |
...
Access tokens expire (15-min TTL). On expiry → server emits `error: { code: "TOKEN_EXPIRED" }`; client refreshes (`Modules/Authentication.md` §6.2) and reconnects. 5s grace prevents disconnect flapping.
...
API authentication follows `Modules/Authentication.md` (the authoritative auth module). What an **API caller** needs to know:
...
| IoT device | X.509 mTLS (MQTT/TCP — `Modules/DeviceGateway.md`) |
...
**Resource-server validation (per `Modules/Authentication.md` §6.3):** signature (RS256, JWKS), `exp`/`nbf`/`iat`, `iss`, `aud`, then Redis revocation (`revocation:<jti>`, `revocation:user:<sub>`). Any failure → `401` generic (no oracle).
...
After authN, Kong + services call **OPA** (`Modules/Authentication.md` §7) with `{ subject, action, resource, context }`. Default-deny. Permissions from the canonical catalog (`02_Domain_Model.md` §6).
...
| `Modules/Authentication.md` (OAuth2, OIDC, JWT, refresh) | §4 |
