...
- **Recommendation:** Define a **platform-wide resilience profile per dependency** in a shared `Documents/05_Resilience_Standards.md`; modules reference it rather than redeclare.
...
- Across `01`, `02`, modules, the service list drifts (14 vs 16 vs 20+). **Recommendation:** a `Documents/06_Service_Registry.md` table: service → context → language → store → owner team → phase → SLO tier. One source of truth.

### DOC-2 🟡 No authoritative Event Catalog (drives INT-1)
- **Recommendation:** `Documents/04_Event_Catalog.md` — every event: producer, topic, schema ref, consumers, version. CI-generated from AsyncAPI + annotations.
...
- `git status` shows only `Documents/` and `Modules/`; the frequently-referenced `/Decisions/ADR-*.md` (12 ADRs) don't exist as files.
...
1. **Build the Event Catalog** (`Documents/04_Event_Catalog.md`) — resolves INT-1, INT-3, INT-5, INT-7, INT-8, DDD-1. Every event: producer, topic (single naming convention), schema, consumers, version. **CI gate.**
...
11. **Service Registry** (`Documents/06_Service_Registry.md`) — DOC-1.
12. **Resilience Standards** (`Documents/05_Resilience_Standards.md`) — INT-6, SEC-6.
