...
> - **`Modules/VideoPlatform.md`** owns the *bounded context policy* — codec/recording modes, evidence integrity, the full REST/gRPC API, personas.
...
HLS is **not** used for live (its 2–10s latency is unacceptable for live monitoring); HLS is reserved for **VOD playback** of recordings (`Modules/VideoPlatform.md` §8). The two paths are deliberately distinct so the player knows which transport it is on.
...
5. **Plugin-free by design.** No Flash, no native plugins — WebRTC + HLS cover all cases (`Modules/VideoPlatform.md` §11.1).
...
Every RTP packet between SFU and viewer is encrypted with **DTLS-SRTP** — the WebRTC-standard key exchange over the DTLS handshake. The media is end-to-end encrypted between the SFU and the browser; the signaling channel (WSS) is separately TLS-encrypted. A network observer sees only encrypted UDP/TCP (`Modules/VideoPlatform.md` §12.1).
...
Built on the platform IAM stack (`Modules/Authentication.md`):
...
| Validation | fail-closed: signature + `exp` + `iss` + `aud` + Redis revocation (`Modules/Authentication.md` §6.3) |
...
A **Video Wall** is a multi-camera grid for ops/security centers — a dispatcher's overview of many cameras/sites/vehicles at once on large monitors (`Modules/VideoPlatform.md` §10.2). This is the most demanding feature: 16 tiles × live WebRTC, sustained for hours, bandwidth-aware.
...
|---|---|
| Layout service | `VideoWallLayout` saved per tenant — tiles + refresh policy (`Modules/VideoPlatform.md` §10.2.1) |
...
|---|---|---|
| Authn | RS256 JWT + revocation (`Modules/Authentication.md` §6) | Kong API Gateway, Socket.IO |
...
Every recorded clip is **hash-chained** for evidentiary admissibility (`Modules/VideoPlatform.md` §12.2, `09` §7.4):
...
### 11.2 Capacity (per `Modules/VideoPlatform.md` §14.2 + `09` §9.2)
...
| `Modules/Authentication.md` (JWT, OPA, AAL) | §5 |
| `09_Video_Gateway.md` (ingest/router — producer) | §1 (consumer), §3, §10 |
| `Modules/VideoPlatform.md` (policy, codec, API, wall) | §2, §6, §8 (referenced) |
...
|---|---|---|---|
| **LV-1** | `POST /streams/batch` (multi-camera open) + `camera-groups` REST surface introduced (§6.5, §9.2) | `Modules/VideoPlatform.md` Appendix B | Add the batch-open + camera-group endpoints to the API reference. |
| **LV-2** | AAL2 (MFA) requirement for `media.video.live` formalized as an OPA step-up policy (§5.1) | `Modules/Authentication.md` §6 / `02` §6 | Confirm the AAL2 policy for live video in the permission catalog. |
...
- **`Modules/VideoPlatform.md`** — owns the **policy**: codec/recording modes, evidence, the full REST/gRPC API, personas. This document executes the live-view subset of that API.
...
- **`Modules/Authentication.md`** — owns **authn mechanics** (JWT, MFA, revocation); this document applies them to the live-video surface with per-stream tokens and OPA step-up.

---

*This Live Video Monitoring Architecture is the canonical consumption-plane reference for the Media & Video context. It is reviewed by the Architecture Review Board alongside `09_Video_Gateway.md` (ingest/router), `Modules/VideoPlatform.md` (policy), and `UI_UX_Design.md` (Video Dashboard). Frontend implementation lives under `web-app/src/features/video/` (`VideoDashboard`, `VideoTile`, `SplitScreen`, `useStream`); the signaling/session/token contracts are governed by `media-service` (`Modules/VideoPlatform.md`) and `Modules/Authentication.md`.*
