...
This sprint delivers the gateway **core**: the contracts and pipeline every protocol plugs into, with one real adapter (GT06) + a stub plugin to prove the path end-to-end. The other six protocols (JT808, JT1078, Teltonika, Meitrack, Concox, Queclink) are later-sprint adapter modules against the same `ProtocolAdapter` contract.
...
| `GATEWAY_LISTENERS` | `gt06:tcp:5016,stub:tcp:5099` | `<adapterId>:<tcp\|udp>:<port>` entries |
...
src/
  ...
  infrastructure/
    ...
    adapters/             # GT06 (reference) + stub (plugin/test) — one module per vendor
