...
> **Relationship to foundation.** This module owns the **Media & Video bounded context** (`02_Domain_Model.md` §1 Context 8; aggregates `VideoChannel`, `Recording`, `StreamSession`, `EventClip`, `AIAlert`). It supersedes the v1.0.0 `docs/modules/VideoPlatform.md` (which was fleet-dashcam-only). v2.0.0 broadens scope to a full **enterprise video surveillance platform** spanning fixed-site CCTV, NVR-class recording, and in-vehicle dashcams — unified on one media backbone. Conforms to ADR-002 (Kafka), ADR-006 (Kotlin + Go + Python split), ADR-007/008 (PostgreSQL + S3 polyglot), ADR-013 (Media & Video context), ADR-015 (Socket.IO canonical real-time), ADR-016 (single topic-naming convention). It consumes camera control from `docs/modules/DeviceGateway.md` §2 (JT1078) and feeds evidence clips to `docs/modules/Compliance-Safety.md` (incidents) and `docs/modules/Notification-Alerting.md`. Reuses the `media` PostgreSQL schema defined in `03_Database_Architecture.md` §6.
...
JT1078 is the **companion media plane** to JT808 (the control channel), terminated by `device-gateway-service` (`docs/modules/DeviceGateway.md` §2):
...
- **Signaling** (WebSocket) over WSS with platform JWT (per `docs/modules/Authentication.md`); media-server rejects any WebRTC negotiation whose `streamSessionId` wasn't issued by media-service.
...
- Pipeline: PyTorch + MLflow (`docs/modules/Analytics-Reporting.md`); TensorRT for NVIDIA; ONNX for portability.
...
| `docs/modules/DeviceGateway.md` §2 (JT1078) | §4.4 |
| `docs/modules/Authentication.md` (JWT, real-time token) | §12.1 |
| `docs/modules/GPSEngine.md` §10 (behavior → trigger) | §6.4 |
| `docs/modules/Compliance-Safety.md` (incidents) | §6.4 (evidence linkage) |
| ADR-002, ADR-006, ADR-007/008, ADR-013, ADR-015, ADR-016 | Throughout |

---

*This Video Surveillance Platform module owns the Media & Video bounded context. Maintained alongside `docs/modules/DeviceGateway.md` (JT1078/JT808 control plane) and `03_Database_Architecture.md` §6 (`media` schema). Consistent with the v2.0.0 foundation. Service code: `media-service` (Kotlin), `media-server` (Go), `video-ai-engine` (Python).*
