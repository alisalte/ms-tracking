...
This section provides **column-level DDL for the core, highest-value aggregates** (per the chosen depth). The remaining 30+ aggregates are listed at table level in §5.9 and detailed in their `docs/modules/*.md`.
...
| `asset.vehicle_assets`, `.depreciation_schedules`, `.disposal_records`, `.procurement_records` | VehicleAsset, DepreciationSchedule, DisposalRecord, ProcurementRecord | (Detailed in `docs/modules/Asset-Lifecycle.md`) |
