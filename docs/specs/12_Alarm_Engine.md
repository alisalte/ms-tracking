...
### 1.5 Latency & Throughput Budgets (`docs/modules/Alarm-Engine.md` §1.5)
...
3. **Fail safe.** On degradation, raw CRITICAL events still flow to delivery as best-effort alerts — never silently drop a safety event (`docs/modules/Alarm-Engine.md` §6.4).
...
| Rule type | `absence` (`docs/modules/Alarm-Engine.md` §1.4 ALM-FR-02) |
...
### 2.11 Severity Matrix (`docs/modules/Alarm-Engine.md` §1.6)
...
Owned by `docs/modules/Alarm-Engine.md` §4.1. The rule is the unit of configuration; one rule = one alarm type for one scope.
...
### 3.3 Rule Condition Types (`docs/modules/Alarm-Engine.md` §1.4 ALM-FR-02)
...
Every suppression emits `notification.alert.suppressed.v1` (for analytics, not delivery) so the noise dashboard can show *what would have fired* and tune rules. Ack/contest feedback from operators feeds back into rule tuning (`docs/modules/Alarm-Engine.md` §1.4 ALM-FR-12).

### 4.4 Concurrency Model (`docs/modules/Alarm-Engine.md` §10.3)
...
Owned by `docs/modules/Alarm-Engine.md` §4.2. The `Alert` is the raised instance — one per (rule, entity, dedup-window). It is *not* event-sourced (high volume; transitions append to `alert_acknowledgements` for audit instead).
...
Owned by `docs/modules/Alarm-Engine.md` §4.2. Reproduced and extended here as the canonical state diagram (§8).
...
An **EscalationPolicy** is a time-based chain: if a CRITICAL (or tenant-configured) alert is not acknowledged within `step.delaySeconds`, it advances to the next step — broader recipients, louder channels. Owned by `docs/modules/Alarm-Engine.md` §4.3.
...
> **CRITICAL bypasses quiet hours.** A 3 AM SOS must wake someone; a 3 AM MINOR idle note must not. The severity matrix + escalation policy encode this; the delivery tier honors it (`docs/modules/Notification-Alerting.md`).
...
The canonical `Alert` state machine — owned by `docs/modules/Alarm-Engine.md` §4.2, extended here with the trigger annotations and the terminal/escalation semantics.
...
### 8.2 State Invariants (`docs/modules/Alarm-Engine.md` §4.2)
...
### 10.1 Load Profile (`docs/modules/Alarm-Engine.md` §10.1)
...
### 10.4 Failure Modes (`docs/modules/Alarm-Engine.md` §10.4)
...
2× headroom (vision guardrail); evaluation path load-tested at 10× projected; chaos tests (Redis kill) quarterly. **Noise ratio KPI target: ≤ 20%** (suppressed/raised) — a healthy alarm system, not a noisy one. The noise dashboard (`docs/modules/Alarm-Engine.md` §9.4) drives continuous tuning.
...
| `docs/modules/Alarm-Engine.md` (domain model, aggregates, API, DDL) | §3, §6 (referenced, not duplicated) |
| `docs/modules/Notification-Alerting.md` (parent — delivery) | §1.3, §6.4 |

### 11.3 Open Items Raised by This Document

| ID | Item | Affected doc | Action |
|---|---|---|---|
| **ALM-1** | `notification-service` runtime is **Node/NestJS/TS** (ADR-021), not Kotlin/Spring (ADR-006) | `docs/modules/Alarm-Engine.md` v2.0.0 header + §10 | Update module header to Node/TS in next revision; domain/algorithm content unchanged. |
| **ALM-2** | Noise-suppression feedback source is **Timescale continuous aggregates** (ADR-022), not ClickHouse | `docs/modules/Alarm-Engine.md` §1 header, §9.4 | Update module to Timescale; ClickHouse re-enters only on the `03` §24.3 trigger. |
| **ALM-3** | Alarm-type catalog (SOS/overspeed/geofence/offline/fuel-theft/temperature/collision/camera) formalized as seeded templates (§2) | `docs/modules/Alarm-Engine.md` §3.5, §5.1 | Add the eight-type template catalog to the module's rule-template library. |
...
| **ALM-5** | CRITICAL force-raise semantics (bypass dedup/rate-limit for safety) formalized (§4.1, §9.2) | `docs/modules/Alarm-Engine.md` §6.4 | Document the force-raise path explicitly in the module's fail-safe section. |

### 11.4 Relationship to Companion Documents

- **`docs/modules/Alarm-Engine.md`** — owns the **domain model**: `AlertRule` / `Alert` / `EscalationPolicy` aggregates, rule types, dedup/correlation algorithms, REST/gRPC API, DDL. This document is the *engine architecture* layer above it.
- **`docs/modules/Notification-Alerting.md`** — owns **delivery**: channels (email/SMS/push/in-app/pager/webhook), user preferences, quiet hours. This engine raises; the parent delivers.
...
*This Alarm Engine Architecture is the canonical event-to-alert computation reference for the Notification & Alerting context. It is reviewed by the Architecture Review Board alongside `docs/modules/Alarm-Engine.md` (domain model), `docs/modules/Notification-Alerting.md` (delivery), and `03_Database_Architecture.md` (`notification` schema). Engine implementation lives under `notification-service/src/modules/alarm-engine/` (`RuleEngine`, `EventProcessor`, `EscalationSupervisor`, `AutoResolver`); the aggregates, rule types, and API are governed by `docs/modules/Alarm-Engine.md`.*
