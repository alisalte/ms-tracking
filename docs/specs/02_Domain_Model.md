...
> **Two new contexts** (Media & Video, plus Auth sub-domain aggregates) are reflected here that were missing from prior drafts. Asset Lifecycle aggregates (VehicleAsset, DepreciationSchedule, DisposalRecord, ProcurementRecord) live in `docs/modules/Asset-Lifecycle.md`; Analytics aggregates (Dashboard, ReportDefinition, MLModel, KPIDefinition) live in `docs/modules/Analytics-Reporting.md`.
...
### 7.1 Core Vocabulary (Selection — full glossary per context in `docs/modules/`)
...
| `docs/modules/*.md` | Per-context detailed designs with code-level specifications |
| `docs/adr/ADR-*.md` | ADR-001 (CQRS+ES), ADR-002 (Kafka), ADR-003 (multi-tenancy), ADR-016 (naming), ADR-017 (behavior score) directly implement this model |
