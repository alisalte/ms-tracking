...
Authoritative module: `docs/modules/Authentication.md`. Summary of what every service/operator must know.
...
Per-tenant quotas (vehicles, GPS/s, API/min, storage) enforced real-time by `billing-service` via Redis atomic counters (`docs/modules/Tenant-Management.md`). Breaches → API Gateway rate-limits that tenant.
...
Authoritative: `docs/modules/DeviceGateway.md`, `docs/modules/Telemetry-Device-Management.md`.
...
- **Right to Erasure:** crypto-shredding (destroy tenant KEK) + 90-day buffer + audit preserved anonymized (`docs/modules/Tenant-Management.md` §6.3).
...
Each module's Security section carries a context-specific STRIDE table (e.g., `docs/modules/Authentication.md` §6.8, `docs/modules/VideoPlatform.md` §12). This document owns the platform-level view; modules own the detailed per-context threats.
...
| `docs/modules/Authentication.md` | §3 |
| `docs/modules/Tenant-Management.md` (isolation, erasure) | §5, §10.3 |
