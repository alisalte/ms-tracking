...
- دوربین فقط با بایت `channel` در AB2/AB3 انتخاب می‌شود؛ URL همیشه `live/md300` است (دستگاه پسوند `/1` یا `/2` را دور می‌اندازد).
...
- **AB4** با URL `live/md300/pb` می‌رود. دستگاه این را ACK می‌کند ولی RTMP را روی همان کلید زنده `live/md300` پابلیش می‌کند؛ پلیر همان HLS زنده را می‌بیند. Seek = **AB5** drag؛ توقف = AB5 end. AB8 باینری است و نباید comma-split شود.
